...
C=P/'Captures';C.mkdir(exist_ok=True)
bridgez=min(sc.h(393.3,178),sc.h(393.3,162))+.10
targets=[('01_Base',221.7,249.5,None),('02_Shallow',191,306,None),('03_Fermentation',110,196.7,None),('04_Yeast',58,65,None),('05_Mold',333,171,None),('06_Deep',411.7,109.3,-59),('07_WaterfallPool',143.3,204.5,None),('08_WaterfallMiddle',137,183,None),('09_Breakwall',149,233.3,None),('10_Sentry',101,140,None),('11_BridgeDeck',393.3,170,bridgez+1.1),('12_BridgeUnder',393.3,170,bridgez-6+1.1),('13_CollapseHole',350,222,None),('14_Abyss',418.2,146,None),('15_DeepAltar',436.7,91,-59),('16_BaseInside',221.7,243.3,None)]
ctx={'phase':0,'index':0,'next':0,'samples':[],'start':time.monotonic(),'busy':False}
def done(error=None):
    unreal.unregister_slate_post_tick_callback(ctx['handle']);(P/'FullRuntime.json').write_text(json.dumps({'error':error,'samples':ctx['samples']},indent=2),encoding='utf-8');unreal.log('DEPTH_FULL_RUNTIME_DONE '+str(error))
def tick(dt):
    if ctx['busy']:return
    ctx['busy']=True
    try:
