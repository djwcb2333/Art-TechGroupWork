...
targets=[('06_Deep',411.7,109.3,-59),('15_DeepAltar',436.7,91,-59)]
ctx={'phase':0,'index':0,'next':0,'samples':[],'start':time.monotonic(),'busy':False}
def done(error=None):
    unreal.unregister_slate_post_tick_callback(ctx['handle']);(P/'DeepPolishRuntime.json').write_text(json.dumps({'error':error,'samples':ctx['samples']},indent=2),encoding='utf-8');unreal.log('DEPTH_DEEP_POLISH_DONE '+str(error))
def tick(dt):
    if ctx['busy']:return
    ctx['busy']=True
    try:
...
unreal.log('DEPTH_DEEP_POLISH_ARMED')
