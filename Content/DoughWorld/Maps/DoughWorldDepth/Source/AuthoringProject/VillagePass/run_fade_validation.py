...
P=Path(__file__).parent.parent;sys.path.insert(0,str(P/'CameraCode'))
...
def stable_tick(dt):
    ...
    if p:
        ...
        if state['stable']>=2.0:
            ...
            test.run('/Game/Maps/DoughWorldDepth/Materials/MI_Dough',str(P/'VillagePass/CameraFadeRuntime.json'))
            return
    if time.monotonic()-state['start']>25:
        unreal.unregister_slate_post_tick_callback(state['handle'])
        (P/'VillagePass/CameraFadeRuntime.json').write_text(json.dumps({'passed':False,'error':'Player never reached stable ground before camera comparison'}),encoding='utf-8')
