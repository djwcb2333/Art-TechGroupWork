...
def save(error=None):
    unreal.unregister_slate_post_tick_callback(ctx['handle']);(P/'VillagePass/WalkRuntime.json').write_text(json.dumps({'error':error,'scope':'actual locomotion between original flour proxy and three base facilities; collection inventory not implemented','legs':ctx['legs']},indent=2),encoding='utf-8');unreal.log('DEPTH_WALK_DONE '+str(error))
def tick(dt):
    if ctx['busy']:return
    ctx['busy']=True
    try:
