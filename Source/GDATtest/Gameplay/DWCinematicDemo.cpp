...
namespace
{
 ...
 UBlueprint* CutBP(const FString& Name,UClass* Parent){const FString Path=TEXT("/Game/DoughWorld/Maps/Gameplay/Blueprints/Cinematics/")+Name;if(auto* B=LoadObject<UBlueprint>(nullptr,*(Path+TEXT(".")+Name)))return B;auto* B=FKismetEditorUtilities::CreateBlueprint(Parent,CreatePackage(*Path),*Name,BPTYPE_Normal,UBlueprint::StaticClass(),UBlueprintGeneratedClass::StaticClass());FAssetRegistryModule::AssetCreated(B);FKismetEditorUtilities::CompileBlueprint(B);return B;}
}
#endif
bool UDWCinematicAuthoringLibrary::CreateCinematicAssets()
{
#if WITH_EDITOR
 const FString P=TEXT("/Game/DoughWorld/Maps/Gameplay/UI/Cinematics/WBP_DWCinematicSubtitle");
 ...
 auto* Font=LoadObject<UFont>(nullptr,TEXT("/Game/DoughWorld/Maps/Gameplay/UI/Interaction/Fonts/F_DWHandDrawn.F_DWHandDrawn"));Text->SetFont(FSlateFontInfo(Font,26));
 ...
 auto* HUD=LoadObject<UWidgetBlueprint>(nullptr,TEXT("/Game/DoughWorld/Maps/Gameplay/UI/WBP_DWGameplay.WBP_DWGameplay"));if(!HUD||!HUD->WidgetTree)return false;
